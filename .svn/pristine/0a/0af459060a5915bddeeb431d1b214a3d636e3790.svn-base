...
@Slf4j
@Service
public class GmailRefreshCommonServiceImpl implements GmailRefreshCommonService {
	...
	public ServiceResult allRefresh(OAuthVO oAuth, String empMail) {
		// CountDownLatch 설정 (비동기 작업 개수만큼 카운트를 설정)
		CountDownLatch latch = new CountDownLatch(4); // 예시로 4개의 비동기 작업 처리한다고 가정

		// 비동기 작업 시작
		new Thread(() -> getRecievedMailList(latch, oAuth, empMail)).start();
		new Thread(() -> getSentMailList(latch, oAuth, empMail)).start();
		new Thread(() -> getDraftMailList(latch, oAuth, empMail)).start();
		new Thread(() -> getDeleteMailList(latch, oAuth, empMail)).start();
	...
	public void getRecievedMailList(CountDownLatch latch, OAuthVO oAuth, String myEmpMail) {
		try {
			// 1. api 호출을 위한 url설정과 oauth 정보 확인

			// 사용 가능한 access token을 발급받은 oauth 객체임!
			OAuthVO myOAuth = googleOAuthServie.getUsableAccessToken(oAuth, myEmpMail);
...
//			 모든 리스트를 매번 다 읽어올 필요는 없기 때문에 마지막 api 호출시간을 불러옴
			String apiCallTime = rMailService.readLastApiCallTime(empMail);

			if (apiCallTime != null) {
			// 데이터 베이스에서 조회한 내용이 있을 때는 쿼리 파라미터로 보내야함
			...
			// 쿼리 뒤에 api call time 설정
			// 기존 쿼리 뒤에 띄어쓰기 한 번 하고 " after:시간"으로 추가하면 됨
			// 띄어쓰기가 url 보낼 때는 인식되지 않기 때문에 +로
				inboxQuery.append(" newer_than:80d");
				inboxQuery.append(String.format(" after:%s %s", unixTimestamp, "newer_than:1000d"));
			}
			 */

			inboxQuery.append(" newer_than:7d");
			try {
	 ...
	 * @param event
	 */
	public void getSentMailList(CountDownLatch latch, OAuthVO oAuth, String myEmpMail) {
		try {
			// oauth 객체
			OAuthVO myOAuth = googleOAuthServie.getUsableAccessToken(oAuth, myEmpMail);
			// 받은 메일함을 불러오는 api의 쿼리파라미터(쿼리 파라미터에 따라 조건이 달라짐)
...
//			}
			...
			sentQuery.append(" newer_than:7d");
			try {
				// google api 호출(받은 메일 리스트를 담은 response 리턴)
	 ...
	 * @param event
	 */
	public void getDraftMailList(CountDownLatch latch, OAuthVO oAuth, String myEmpMail) {
		try {
			// oauth 객체
			OAuthVO myOAuth = googleOAuthServie.getUsableAccessToken(oAuth,myEmpMail);
			String empMail = myOAuth.getOauthEmpmail(); // 메일 주소
...
//			}
			draftQuery.append(" newer_than:7d");
...
//						dmailDraftId를 통해 mailMessageId에 접근,
	 ...
	 * @param event
	 */
	public void getDeleteMailList(CountDownLatch latch, OAuthVO oAuth, String myEmpMail) {
		try {
			// oauth 객체
			OAuthVO myOAuth = googleOAuthServie.getUsableAccessToken(oAuth,myEmpMail);
			// 받은 메일함을 불러오는 api의 쿼리파라미터(쿼리 파라미터에 따라 조건이 달라짐)
...
//				sentQuery.append(String.format(" after:%s %s", unixTimestamp, "newer_than:1000d"));
//			}
			deleteQuery.append(" newer_than:7d");
			try {
				// google api 호출(받은 메일 리스트를 담은 response 리턴)
